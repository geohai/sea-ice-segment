...
class DeepLabLikeMultiStreamDict(pl.LightningModule):
    def __init__(self, *args, **kwargs):
        ...
        for _ in range(self.hparams.num_streams):
            ...
            backbone.conv1 = nn.Conv2d(4, 64, kernel_size=7, stride=2, padding=3, bias=False) # Change input channels to 4
    ...
    def forward(self, x:List[Tensor]) -> Tensor:
        ...
        x = [t.unsqueeze(0) if t.dim() == 3 else t for t in x]
        x = torch.cat(x, dim=1)
        
        _features = []
        # need enumerate for torch.jit.script 
        for stream in self.streams:
            _features.append(stream(x)['out'])
